...
export function StepImages({ onNext, onPrev }: { onNext: () => void; onPrev: () => void }) {
  ...
  async function uploadFiles(files: File[]) {
    ...
    try {
      for (const file of toUpload) {
        ...
        setValue('images', reindex([...watch('images'), { url: result.url, index: 0 }]));
        setPendingCount((prev) => prev - 1);
      }
    } finally {
      setUploading(false);
  ...
  function handleRemove(index: number) {
    setValue('images', reindex(images.filter((img) => img.index !== index)));
  }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    setValue('images', reindex(arrayMove(images, oldIndex, newIndex)));
  }
